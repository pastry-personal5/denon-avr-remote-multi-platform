...
use denon_avr_core::{
    AsyncReceiverDiscovery, BoxFuture, DiscoveredReceiver, OperationError, OperationErrorKind,
    ReceiverDiscovery,
};
...
impl ReceiverDiscovery for SsdpDiscovery {
    fn discover(&self, timeout: Duration) -> Result<Vec<DiscoveredReceiver>, OperationError> {
        discover(timeout).map_err(discovery_operation_error)
    }
}

impl AsyncReceiverDiscovery for SsdpDiscovery {
    fn discover(
        &self,
        timeout: Duration,
...
async fn discover_async(timeout: Duration) -> Result<Vec<DiscoveredReceiver>, DiscoveryError> {
    let interface_ips = local_private_ipv4_addresses()?;
    let mut sockets = Vec::new();
    for interface_ip in &interface_ips {
    ...
    let deadline = AsyncInstant::now() + timeout;
    let retry_at = AsyncInstant::now() + timeout / 2;
    ...
    if receivers.is_empty() {
        return discover_by_description_scan_async(&interface_ips).await;
    }

    let mut enriched = Vec::with_capacity(receivers.len());
    for receiver in receivers.into_values() {
        enriched.push(enrich_receiver_async(receiver).await);
    }
    Ok(enriched)
}
...
async fn discover_by_description_scan_async(
    interface_ips: &[Ipv4Addr],
) -> Result<Vec<DiscoveredReceiver>, DiscoveryError> {
    ...
    for candidate in candidates {
        ...
        tasks.spawn(async move {
            ...
            probe_description_async(candidate).await
        });
    }
...
async fn probe_description_async(ip: Ipv4Addr) -> Option<DiscoveredReceiver> {
    let address = SocketAddr::from((ip, DESCRIPTION_PORT));
    let mut stream = tokio::time::timeout(SCAN_CONNECT_TIMEOUT, AsyncTcpStream::connect(address))
        .await
        .ok()?
        .ok()?;
    let request = format!(
    ...
    tokio::time::timeout(SCAN_READ_TIMEOUT, stream.write_all(request.as_bytes()))
        .await
        .ok()?
        .ok()?;

    let mut response = Vec::new();
    let read = tokio::time::timeout(SCAN_READ_TIMEOUT, stream.read_to_end(&mut response)).await;
    match read {
        Ok(Ok(_)) | Err(_) if !response.is_empty() => {}
        _ => return None,
    }
    if response.len() > 65_536 {
        response.truncate(65_536);
    }
    let text = std::str::from_utf8(&response).ok()?;
    let (_, body) = text
        .split_once("\r\n\r\n")
...
async fn enrich_receiver_async(mut receiver: DiscoveredReceiver) -> DiscoveredReceiver {
    let IpAddr::V4(ip) = receiver.address.ip() else {
        return receiver;
    };
    let Some(description) = probe_description_async(ip).await else {
        return receiver;
    };
    receiver.model = description.model.or(receiver.model);
    receiver.location = description.location.or(receiver.location);
...
fn receiver_key(receiver: &DiscoveredReceiver) -> (String, Option<String>) {
    (receiver.address.ip().to_string(), receiver.location.clone())
}

fn is_denon_family(receiver: &DiscoveredReceiver) -> bool {
    [
