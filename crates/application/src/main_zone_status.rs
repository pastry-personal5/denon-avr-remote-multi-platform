//! Main zone status query use case.

use crate::ports::{AsyncStatusGateway, OperationError, OperationErrorKind, StatusGateway};
...
pub fn query_main_zone_status(gateway: &mut impl StatusGateway) -> MainZoneSnapshot {
    ...
    let snapshot = query_main_zone_status_once(gateway);
    if gateway.connection_generation() != generation {
        return query_main_zone_status_once(gateway);
    }
    snapshot
}

fn query_main_zone_status_once(gateway: &mut impl StatusGateway) -> MainZoneSnapshot {
...
pub async fn query_main_zone_status_async(
    gateway: &mut impl AsyncStatusGateway,
) -> MainZoneSnapshot {
    let generation = gateway.connection_generation();
    let snapshot = query_main_zone_status_async_once(gateway).await;
    if gateway.connection_generation() != generation {
        return query_main_zone_status_async_once(gateway).await;
    }
    snapshot
}

async fn query_main_zone_status_async_once(
    gateway: &mut impl AsyncStatusGateway,
) -> MainZoneSnapshot {
    let mut snapshot = MainZoneSnapshot::default();
    for field in MainZoneField::ALL {
...
fn field_error(error: OperationError) -> FieldError {
    let kind = match error.kind {
        ...
        OperationErrorKind::Unavailable => FieldErrorKind::Unavailable,
...
mod tests {
    ...
    use denon_avr_domain::{Input, MainZoneValue, MuteState, PowerState, SurroundMode, Volume};
    ...
    fn preserves_independent_field_failures() {
        ...
        let status = query_main_zone_status(&mut fake);
