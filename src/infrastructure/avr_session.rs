    ...
    fn query_field(
        ...
        Box::pin(async move {
            ...
            parse_main_zone_response(field, &response).map_err(|error| {
                let kind = if matches!(
                    error,
                    crate::protocol::avr::AvrProtocolError::Unavailable(_)
                ) {
...
fn apply_response(snapshot: &Arc<Mutex<MainZoneSnapshot>>, family: &str, response: &str) {
    ...
    match crate::protocol::avr::parse_main_zone_response(field, response) {
        Ok(value) => {
            if let Ok(mut state) = snapshot.lock() {
                state.set_value(value, StateAuthority::Authoritative);
            }
        }
        Err(crate::protocol::avr::AvrProtocolError::Unavailable(message)) => {
            if let Ok(mut state) = snapshot.lock() {
                state.set_error(
                    field,
                    FieldError {
    ...
    if let Ok(mut state) = snapshot.lock() {
        match authority {
            StateAuthority::Event => {
                state.apply_event(crate::protocol::avr::parse_main_zone_event(line))
            }
            StateAuthority::Authoritative => {
                state.apply_authoritative_event(crate::protocol::avr::parse_main_zone_event(line))
            }
